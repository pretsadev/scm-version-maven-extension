...
public class ConfigurationBuilder {
     ...
     * @param topDirectory repository path
     * @param properties input properties
     * @return configuration instance
     */
    public static Configuration build(Path topDirectory, Map<String, String> properties) {
        logger.atDebug().addArgument(topDirectory).log("Working directory is {}");

        VersionType versionType = getVersionType(properties.keySet());
        SemanticVersion.Component nextVersionComponent = getNextVersionComponent(versionType, properties.keySet());
    ...
    private static SemanticVersion.Component getNextVersionComponent(VersionType versionType, Set<String> keys) {
        ...
        if (containsMajor) {
            logUserDefinedProperty(NEXT_VERSION_COMPONENT, SemanticVersion.Component.MAJOR.name());
            return SemanticVersion.Component.MAJOR;
        }
        if (containsMinor(keys)) {
            logUserDefinedProperty(NEXT_VERSION_COMPONENT, SemanticVersion.Component.MINOR.name());
            return SemanticVersion.Component.MINOR;
        }
        if (containsPatch(keys)) {
            logUserDefinedProperty(NEXT_VERSION_COMPONENT, SemanticVersion.Component.PATCH.name());
            return SemanticVersion.Component.PATCH;
        }

        logDefaultProperty(NEXT_VERSION_COMPONENT, SemanticVersion.Component.PATCH.name());
        return SemanticVersion.Component.PATCH;
    }
